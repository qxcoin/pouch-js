...
test('can retrieve block height', async () => {
  const wallet = new TronWallet('radar blur cabbage chef fix engine embark joy scheme fiction master release', 'testnet', { provider: 'https://api.trongrid.io', apiKey: 'f1c685a8-97ee-4a4f-8475-381fc2a4278b' });
  const height = await wallet.getLastBlockHeight();
  expect(typeof height).toBe('number');
});

test('can create valid address', async () => {
  const wallet = new TronWallet('radar blur cabbage chef fix engine embark joy scheme fiction master release', 'mainnet', { provider: 'https://example.com', apiKey: 'example' });
